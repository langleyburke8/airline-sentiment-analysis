...
def encode_ST_messages(reviews, vocab_to_int):
    """
    ...
    for Review in reviews:
        reviews_encoded.append([vocab_to_int[word] for word in reviews.split()])

    return np.array(reviews_encoded, type=object)
...
def encode_ST_labels(labels):
    ...
    """
    return np.array([1 if sentiment == 'yes' else 0 for sentiment in labels])

def drop_empty_messages(reviews, labels):
    """
    ...
    reviews_non_zero = np.array([reviews[ii] for ii in non_zero_idx])
    labels_non_zero = np.array([labels[ii] for ii in non_zero_idx])
    return reviews_non_zero, labels_non_zero

def zero_pad_messages(reviews, seq_len):
    """
    ...
    :return: numpy array.  The encoded labels
    """
    reviews_padded = np.zeros((len(reviews), seq_len), dtype=int)
    for i, row in enumerate(reviews):
        reviews_padded[i, -len(row):] = np.array(row)[:seq_len]

    return np.array(reviews_padded)

def train_val_test_split(reviews, labels, split_frac, random_seed=None):
    """
    Zero Pad input messages
    :param messages: Input list of encoded messages
    ...
    shuf_idx = np.random.permutation(len(reviews))
    reviews_shuf = np.array(reviews)[shuf_idx] 
    labels_shuf = np.array(labels)[shuf_idx]

    #make splits
    split_idx = int(len(reviews_shuf)*split_frac)
    train_x, val_x = reviews_shuf[:split_idx], reviews_shuf[split_idx:]
